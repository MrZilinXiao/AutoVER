...
import os
import pathlib
from LLaVA.llava.model.language_model.llava_llama import LlavaLlamaForCausalLM, LlavaLlamaForCausalLMWithRet
...
class DataArguments:
    ...
    query_jsonl_path: str = field(default=None)  # if enabled, will mix with train_jsonl_path
    query_sample_ratio: int = field(default=1)
...
def train():
    ...
    if model_args.freeze_backbone:  # training LM or not
    ...
    if model_args.retrieval:
        ...
        model.get_model().move_retrieval_to_device(dtype=torch.bfloat16 if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        # TODO: update grad control for entity_encoder here

        num_added_tokens = model.initialize_retrieval_tokenizer(model_args, tokenizer=tokenizer)  # add token & expand vocab
